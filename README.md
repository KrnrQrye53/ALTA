...
ID が `AI-` で始まり `"source": "ai"` を持つ問題は、シラバス v3.1.1.J03 本文を根拠に AI が作成し、別の AI が解き直して検証したものです。正解の根拠は解説中のシラバス節・ページで確認できます。誤りに気付いたら修正してください。

解説で選択肢に触れるときは「1番目」「Aは」のような位置ではなく、選択肢の文言を「」で引用してください（表示時に選択肢の順番がシャッフルされるため）。
...
```jsonc
{
  ...
  "explanation": "解説…",
  "source": "ai"             // AI作成問題のみ（任意）。画面に「AI作成」チップが出る
}
```
